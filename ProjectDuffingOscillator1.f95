...
program ProjectDuffingOscillator
    ...
    !Simulation type
    integer, parameter  ::  sTY = 1  !1 for constant time step, 2 for adaptive time step
    
    !Data for Time Series, Phase Plot and Poincare sections, i.e [time: t, displacement: x1 and velocity: x2]
    logical, parameter  ::  gTXdata = .true. !if true, then get data [t, x1, x2] at the points in variable 'pT'
    ...
    real (KIND=PRECS), parameter  :: x1_i = 1.0_PRECS  !Initial Displacement
    ...
    real (KIND=PRECS), parameter  :: Po = 0.1_PRECS         !Forcing amplitude
    ...
    real (KIND=PRECS), parameter  :: a1 = -0.5_PRECS, bt = 0.5_PRECS                !other duffing equation parameters
    !real (KIND=PRECS), parameter  :: a1 = -1_PRECS, bt = 1_PRECS                !other duffing equation parameters
    ...
    real (KIND=PRECS), parameter  ::  nT = 5000.0_PRECS      !number of excitation periods
    ...
    real (KIND=PRECS), parameter  :: uST = 50.0_PRECS*T1        !unsteady period of oscillation. Any period after is steady
    ...
    !Output files
    open ( unit=21, file= 'results\1otDatFl_w0_x0_s0.csv') !output file for no of steps for each runge kutta version [55 versions]
    if(gTXdata) open ( unit=22, file= 'results\1otTXXFl_w0_x0_s0.csv') !output file for t, x1 and x2  [optional]
    ...
    if(gTXdata) write (22, fmt = "(1X, A5, ',', 12(A5,',',E15.6,','))") 'ALL:','t_i:',t_i,'x1_i:',x1_i,'x2_i:',x2_i, &
                    'w:',w,'T1:',T1,'nT1:',nT,'T_t:',T_t,'h:',h_i,'Sim:',float(sTY),'UnSt:',uST/T1,'Eq:',Eq,'PRES',float(PRECS);
    ...
    do i = 1,1 !Run for each damping constant value
        dC_1 = 0.0168 !calculate for the current iteration
        
        do j = 1,1 !Run for each amplitude value
            Po_1 = 0.09; !calculate for the current iteration
            
            do k= 55,55 !Run for each Runge Kutta version
                ...
                do ! [endless do] Run while total simulation time is greater than current time (t <= T_t) 
                    !if(mod(nOfTSs(k),int(nT))==0) print *, 'Step:', nOfTSs(k); ! '.....................';
                    ...
                    if (sTY == 2) then !only if adaptive step size is to be used
                        ...
                        do p = 1,2 !since we are using step halving, run twice.
                            ...
                            !Step 1 is not needed here. Simply reuse values from the previous Step 1
                    ...
                    !if (cnd .and. (t > uST) .and. (mod(t,T1)>=0.0 .and. mod(t,T1)< h) ) &
                    !.and. (mod(t,T1)>=0.0 .and. mod(t,T1)< h)
                    if (cnd .and. (mod(t,T1)>=0.0 .and. mod(t,T1)< h) ) &
                        write (22, fmt = "(1X, 2(I16, ','), 3(E15.6, ','))") nOfTSs(k),nOfTSd(k), t, x1, x2; !T1, h, mod(t,T1)
